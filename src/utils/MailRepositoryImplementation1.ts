...
describe('getMailsByDateAndId', () => {
    it('should get Mails from a User on a date', async () => {
        ...
        expect(mails[0].id).toEqual(1);
        expect(mails[1].id).toEqual(2);
...
describe('getMailsByUserId', () => {
    it('should get Mails from a User by the id provided', async () => {
        ...
        const userId = 1;
        mailEntity2 = {... mailEntity2, userId: 2}
        
        expect.assertions(3);

        prismaMock.mailEntity.findMany.mockResolvedValue([mailEntity]);
        const mails: Mail[] = await mailRep.getMailsByUserId(userId);

        expect(mails).toHaveLength(1); // Must be two emails within the date range
        expect(mails[0].id).toEqual(1);
